...
namespace OpenData
{
    ...
    /// </summary>
    ///
    [Guid("4FA1EB1C-5AF3-451F-BF1B-33E794FD62C0")]
    public partial class MainWindow
    {
        ...
        private const double BxlLatitude = 50.848893;
        private const double BxlLongitude = 4.349396;
        ...
        public MainWindow()
        {
            ...
            Trace.WriteLine(DateTime.Now + " " + "Démarrage de OpenData - villo.stations...");
            ...
            MainMap.MinZoom = 10;
            MainMap.Zoom = 14;
            MainMap.CacheLocation = GetFolderPath(SpecialFolder.CommonApplicationData) + "\\brussels open data - villo.stations\\cache\\";
        ...
        private void DownloadClient()
        {
            ...
            try
            {
                ...
                webClient.DownloadFileAsync(
                    new Uri("https://api.jcdecaux.com/vls/v1/stations?apiKey=6d5071ed0d0b3b68462ad73df43fd9e5479b03d6&contract=Bruxelles-Capitale"),
        ...
        private void PushpinMouseDown(object sender, MouseButtonEventArgs e)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(ab[tag].number.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelStationNumber.Content = "Station Number: " + ab[tag].number;
                }
                else
                {
                    LabelStationNumber.Content = "Station Number: no data";
                }
                ...
                {
                    LabelStationName.Content = "Station Name: no data";
                }

                if (!string.IsNullOrEmpty(ab[tag].position.lat.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelLatitudeDegrees.Content = "Latitude Degrees: " +
                                                   ab[tag].position.lat;

                    _latitudePushpinClicked = ab[tag].position.lat;
                }
                else
                {
                ...
                if (!string.IsNullOrEmpty(ab[tag].position.lng.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelLongitudeDegrees.Content = "Longitude Degrees: " +
                                                    ab[tag].position.lng;

                    _longitudePushpinClicked = ab[tag].position.lng;
                }
                else
                {
                ...
                {
                    LabelAdress.Content = "Address: no data";
                }

                if (!string.IsNullOrEmpty(ab[tag].banking.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelCreditCard.Content = "Credit Cards: " + ab[tag].banking;
                }
                else
                {
                    LabelCreditCard.Content = "Credit Cards: no data";
                }

                if (!string.IsNullOrEmpty(ab[tag].bonus.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelBonus.Content = "Bonus: " +
                                            ab[tag].bonus;
                }
                else
                {
                    LabelBonus.Content = "Bonus: no data";
                }

                if (ab[tag].status != null)
                {
                    LabelStatus.Content = "Status: " + ab[tag].status;
                }
                else
                {
                    LabelStatus.Content = "Status: no data";
                }

                if (ab[tag].contract_name != null)
                {
                    LabelContractName.Content = "Contract Name: " +
                                              ab[tag].contract_name;
                }
                else
                {
                    LabelContractName.Content = "Contract Name: no data";
                }

                if (!string.IsNullOrEmpty(ab[tag].bike_stands.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelTotalCapacity.Content = "Total Capacity: " + ab[tag].bike_stands;
                }
                else
                {
                    LabelTotalCapacity.Content = "Total Capacity: no data";
                }

                if (!string.IsNullOrEmpty(ab[tag].available_bike_stands.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelAvailableBikeStands.Content = "Available Bike Stands: " +
                                              ab[tag].available_bike_stands;
                }
                else
                {
                    LabelAvailableBikeStands.Content = "Available Bike Stands: no data";
                }

                if (!string.IsNullOrEmpty(ab[tag].available_bikes.ToString(CultureInfo.InvariantCulture)))
                {
                    LabelAvailableBikes.Content = "Available Bikes: " +
                                                    ab[tag].available_bikes;
                }
                else
                {
                    LabelAvailableBikes.Content = "Available Bikes: no data";
                }

                if (ab[tag].last_update != null)
                {
                    double str = Convert.ToDouble(ab[tag].last_update);
                    DateTime dt = new DateTime(1970, 1, 1, 0, 0, 0, 0).AddSeconds(Math.Round(str / 1000d));
                    LabelLastUpdate.Content = "Last Update: " + dt.ToLongTimeString() + " UTC";
                }
                else
                {
                    LabelLastUpdate.Content = "Last Update: no data";
                }
        ...
        private void DBOptimization()
        {
            ...
            try
            {
                ...
                "\\brussels open data - villo.stations\\cache\\TileDBv5\\en\\Data.gmdb"))
                {
                    lock (this)
                    {
            ...
            if (!area.IsEmpty && MainMap.IsLoaded)
            {
                ...
                if (CheckNet())
                {
                    for (int i = MainMap.MinZoom; i <= MainMap.MaxZoom; i++)
                    {
                        ...
                        {
                            Title = "brussels open data - villo.stations",
                            Owner = System.Windows.Application.Current.MainWindow,
                            Top = Top,
                            Left = Left,
